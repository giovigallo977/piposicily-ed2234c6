...
const ExplorePage = () => {
  ...
  return (
    <div className="min-h-screen bg-background">
        ...
        <DropdownMenu>
          ...
          <DropdownMenuContent align="end" className="w-48 bg-background border border-border">
            <DropdownMenuItem
              ...
              className={`cursor-pointer font-sans ${!selectedCategory ? "bg-muted" : ""}`}
            >
              {t("allCategories")}
            </DropdownMenuItem>
            {categories.map((category, index) => (
              <DropdownMenuItem
                ...
                className={`cursor-pointer font-sans ${selectedCategory === category ? "bg-muted" : ""}`}
              >
      ...
      {(zonaParam || moodParam || selectedCategory) && (
        <div className="px-6 py-2 flex flex-wrap gap-2 justify-center">
          {zonaParam && (
            <span className="inline-flex items-center gap-1 px-3 py-1.5 rounded-full text-xs font-medium bg-olive text-white font-sans">
              📍 {zonaParam}
            </span>
          )}
          {moodParam && (
          ...
          {selectedCategory && (
            <button
              onClick={() => setSelectedCategory(null)}
              className="inline-flex items-center gap-1 px-3 py-1.5 rounded-full text-xs font-medium bg-black text-white font-sans"
            >
              {selectedCategory}
